...
public class ChromostereopsisProcessor {
    ...
    public Bitmap applyEffect(Bitmap original, float[][] depthMap, EffectParams params) {
        ...
        // edge cropping or ArrayIndexOutOfBounds errors.
        float scaleX = width > 1 ? (float) (mapW - 1) / (width - 1) : 0f;
        float scaleY = height > 1 ? (float) (mapH - 1) / (height - 1) : 0f;
        ...
        for (int y = 0; y < height; y++) {
            int mapY = Math.min(Math.round(y * scaleY), mapH - 1);
            for (int x = 0; x < width; x++) {
                int mapX = Math.min(Math.round(x * scaleX), mapW - 1);
